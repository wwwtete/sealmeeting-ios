...
#不同电脑上不同版本的 pod 执行 pod update 可能会修改 SealMeeting.xcodeproj/project.pbxproj 文件
#必须要重置 pod 对其修改，否则部分电脑运行项目可能会报错
# git checkout -- SealMeeting.xcodeproj/project.pbxproj

#更新版本号
sed -i "" -e '/CFBundleShortVersionString/{n;s/[0-9]\.[0-9]\{1,2\}\.[0-9]\{1,2\}/'"$VER_FLAG"'/; }' ./SealMeeting/Info.plist
sed -i "" -e '/CFBundleVersion/{n;s/[0-9]*[0-9]/'"$CUR_TIME"'/; }' ./SealMeeting/Info.plist

if [ -n "${DEMO_SERVER}" ];then
  sed -i "" -e 's?^NSString \*const.*$?NSString \*const BASE_URL = @\"'${DEMO_SERVER}'\";?' ./SealMeeting/Util/HTTP/HTTPUtility.m
fi

PROJECT_NAME="SealMeeting.xcworkspace"
targetName="SealMeeting"
...
cd $OUTPUT_PATH
zip -r $OUTPUT_PATH/SealMeeting_v${VER_FLAG}_${BUILD_NUMBER}_${CUR_TIME}.app.dSYM.zip ./*.app.dSYM
rm -rf $OUTPUT_PATH/*.app.dSYM
cd ${BUILD_PATH}

function archive_sourcecode() {
  Source_Name="SealMeeting_iOS"
  mkdir $Source_Name
  ...
  cp -af ./SealMeeting $Source_Name
  cp -af ./SealMeeting.xcodeproj $Source_Name
